...
const fetchNDROrdersForUser = async (req, res) => {
  try {
    ...
    const { page = 1, limit = 100 } = req.query; // Pagination support
    const skip = (page - 1) * limit;
  ...
  .populate({
    ...
    populate: {
      path: "orderIds", // this works only if ref is set in Shipping schema
    },
  })
      .sort({ createdAt: -1 })
      .skip(skip)
...
const handleNDRAction = async (req, res) => {
  ...
  try {
    ...
    console.log("Awb to Send xpress bees : ", awb);
...
const handleXpressBees = async (awb, action, action_data) => {
  ...
  const validateActionData = () => {
    switch (action) {
      ...
      case "change_address":
        if (!action_data?.address || !action_data?.address_1) throw new Error("Missing address fields");
  ...
  try {
    ...
    console.log("Error in XpressBees action handler:", error);
    throw new Error(`XpressBees Error: ${error.message || "Action failed"}`);
  }
};
