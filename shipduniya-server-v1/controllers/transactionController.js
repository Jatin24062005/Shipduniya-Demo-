...
const getAllTransactionsOfUser = async (req, res) => {
  const  id  = req.user.id; // Authenticated user ID
  // const { type, page = 1, limit = 100 } = req.query; // Filters and pagination parameters
  ...
  try {
    ...
    const sortTransaction = transactions.sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt));
