        ...
        {data?.filter(item => item.remittanceRequestIds.length>0 && item.status === "paid").sort((a,b)=> new Date(b.createdAt)-new Date(a.createdAt)).map((item, index) => (
            <React.Fragment key={index}>
              <TableRow className="hover:bg-gray-50">
                ...
                <TableCell>{item?.sellerId?._id?.toString() || "N/A"}</TableCell>
                  <TableCell>{item?.sellerId?.name || "N/A"}</TableCell>
                  <TableCell>{item?.remittanceId || "N/A"}</TableCell>
                  <TableCell>{item?.sellerId?.bankDetails[0]?.bankName || "N/A"}</TableCell>
                  <TableCell>{item?.sellerId?.bankDetails[0]?.ifscCode || "N/A"}</TableCell>
                  <TableCell>{item?.sellerId?.bankDetails[0]?.accountType || "N/A"}</TableCell>
