...
import Xpressbess from "./../../../public/xpressbees-icon.png"
import Delhivery from "./../../../public/delhivery-logo.svg"
import Logo from "./../../../public/compass akela logo (1).png"
...
export function TrackingForm({ onTrack, isLoading }) {
  ...
  const courierFromUrl = searchParams.get("courier");
  ...
  useEffect(() => {
    if (awbFromUrl) setAwbNumber(awbFromUrl);
    if (courierFromUrl) setSelectedPartner(courierFromUrl);

    // ✅ Automatically trigger tracking when both are present
    if (awbFromUrl && courierFromUrl) {
      console.log("CourierFromUrl : ",courierFromUrl)
      console.log("awbFromUrl : ",awbFromUrl)
      onTrack(courierFromUrl,awbFromUrl);
    }
  }, [awbFromUrl, courierFromUrl]);
  ...
  return (
    <div ref={formRef} className="w-full min-w-[450px]  max-w-md mx-auto">
      <div className="bg-white border border-gray-200 min-h-[500px] rounded-2xl shadow-xl p-6 space-y-6">
        {/* Logo Section */}
        <div ref={logoRef} className="text-center space-y-2">
          <div className="w-20 h-20  rounded-xl mx-auto flex items-center justify-center">
            ...
            <Image src={Logo} alt='shipduniya.in'  className="w-fit h-20 text-white"/>
          </div>
          <div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          {/* Partner Selection */}
          <div ref={partnersRef} className="space-y-2 mt-10">
