...
const EditOrder = ({ isEditing, setIsEditing, editOrder, setEditOrder }) => {
  ...
  useEffect(() => {
    const updateLocation = async () => {
      if (editOrder?.pincode && editOrder.pincode.length === 6) {
        const { District, State } = await fetchLocation(editOrder.pincode);
        setCity(District || "");
        setState(State || "");
      } else {
        setCity("");
  ...
  useEffect(() => {
    ...
    if (
      editOrder?.orderType.toLowerCase() === "prepaid" &&
      parseFloat(editOrder.collectableValue) > 0
    ) {
        ...
        {step === 1 && (
          <div className="flex gap-6 justify-between flex-wrap">
            ...
            <div className="flex flex-col pb-4 w-[40%]">
              ...
              <input
                type="text"
                value={editOrder?.channelPartner || "Default"}
                onChange={(e) =>
                  setEditOrder({
                    ...editOrder,
