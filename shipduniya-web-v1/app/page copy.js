...
export default function LandingPage() {
  ...
  const handleTrackParcel = async () => {
    ...
    try {
      ...
      // Set track data and open the second dialog
      ...
      setLoading(false);
      // Close the first dialog once tracking is attempted
      setIsTracking(false);
    }
  };
    ...
    <div className="flex flex-col min-h-screen">
      <header className="bg-secondary py-4 px-4 md:px-8 lg:px-16">
        <div className="container mx-auto flex justify-between items-center relative">
          {/* Navigation & Sheet code here */}
          <nav className="hidden md:block ml-[35%]">
              ...
              {navItems.map((item) => (
                <li key={item.name}>
                  <button
                    ...
                    className={
                      activeTab === item.name
                        ? "relative bg-black/80 text-white rounded-lg py-1 px-3"
          ...
          <div className="hidden md:flex space-x-2">
           <Link href="/tracking" target="_blank" rel="noopener noreferrer">
                ...
                <Button
                  ...
                  onClick={() => setIsTracking(false)}  // for using PopUp setIsTracking(true)
                >
      ...
      {/* Second Dialog: Parcel Details (only rendered when track data is available) */}
      {track && (
        <Dialog
          open={isParcelDetailsOpen}
          onOpenChange={(open) => {
            setIsParcelDetailsOpen(open);
            // Clear track data when closing the dialog
