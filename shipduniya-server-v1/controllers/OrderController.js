  ...
  try {
    ...
    if (file && isExcelFile(file)) {
      ...
      worksheet.eachRow((row, rowNumber) => {
        if (rowNumber > 1) {
          ...
          const rowData = {
            ...
            orderNumber: row.getCell(4).value,
            orderType: (row.getCell(5).value || "PREPAID")
              .toString()
              .toUpperCase(),
           
            pincode: row.getCell(6).value,
             channelPartner:row.getCell(7).value,
            insurance: row.getCell(8).value || "No",
            mobile: row.getCell(9).value || user.mobile,
            invoiceNumber:
              row.getCell(10).value ||
              `INV-${Date.now()}-${Math.floor(Math.random() * 1000)}`,
            telephone: row.getCell(11).value || user.telephone,
            city: row.getCell(12).value,
            state: row.getCell(13).value,
            length: row.getCell(14).value,
            breadth: row.getCell(15).value,
            height: row.getCell(16).value,
            collectableValue: parseFloat(row.getCell(17).value) || 0,
            declaredValue: parseFloat(row.getCell(18).value) || 0,
            itemDescription: row.getCell(19).value,
            dgShipment: row.getCell(20).value || false,
            quantity: parseInt(row.getCell(21).value) || 1,
            volumetricWeight: parseFloat(row.getCell(22).value) || 0,
            actualWeight: parseFloat(row.getCell(23).value),
          };
      ...
      const newOrder = new Order({
        ...
        orderId: order.orderNumber || order.orderId, // user orderId
        orderType: ordertype || "PREPAID",
        consignee: order.consignee || user.name,
        consigneeAddress1: order.consigneeAddress1,
        consigneeAddress2: order.consigneeAddress2,
        city: order.city,
        channelPartner: order.channelPartner || "Default",
        insurance: order.insurance || "No",
        state: order.state,
        pincode: order.pincode,
...
const editOrderByUser = async (req, res) => {
  try {
    ...
    // Normalize orderType to uppercase if present
    if (orderUpdates.orderType) {
      orderUpdates.orderType = orderUpdates.orderType.toUpperCase();
    }
    if (req.file) {
      orderUpdates.image = {
    ...
    const updatedOrderType = orderUpdates.orderType || existingOrder.orderType;
    const updatedDeclaredValue =
      orderUpdates.declaredValue !== undefined
        ? orderUpdates.declaredValue
        : existingOrder.declaredValue;

    let updatedCollectableValue =
      orderUpdates.collectableValue !== undefined
        ? orderUpdates.collectableValue
        : existingOrder.collectableValue;

    // ✅ Business Logic Validation
    if (updatedOrderType === "COD") {
      if (updatedCollectableValue <= 0) {
        return res.status(400).json({
          error: "COD orders must have a collectable value greater than 0.",
        });
      }

      if (updatedCollectableValue > updatedDeclaredValue) {
        return res.status(400).json({
          error:
            "Collectable value cannot exceed declared value for COD orders.",
        });
      }
    } else if (updatedOrderType === "PREPAID") {
      // Auto-fix collectableValue
      updatedCollectableValue = 0;
    }
    ...
    const updatedOrder = await Order.findOneAndUpdate(
      ...
      {
        ...orderUpdates,
        collectableValue: updatedCollectableValue,
        shipped: false,
      },
  ...
  try {
    ...
    if (Array.isArray(req.body.orderIds) && req.body.orderIds.length > 0) {
      ...
      const reverseOrders = originalOrders.map((originalOrder) => ({
        ...
        insurance: originalOrder.insurance || "No",
        channelPartner: originalOrder.channelPartner || "Default",
